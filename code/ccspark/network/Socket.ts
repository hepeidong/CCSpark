...
import { cc_spark_socket_message, cc_spark_socket_protocol, Constructor, ICCSocketManager, IResponseSocketData, ISocket, ISocketChange, ISocketData, ISocketMessage, ISocketMessage_extend, ISocketNetworkDelay, ISocketNotConnected, ISocketProtocol, SocketChange, SocketNetworkDelay, SocketNotConnected } from "../lib.ccspark";
import { js } from "cc";
import { EventSystem } from "../event";
import { tools } from "../tools";
import { errMsg, ProtocolType, SocketEnum } from "./SocketEnum";
...
export class CCSocket implements ICCSocketManager {
    ...
    private _sendQueue: tools.Queue<any>;
    private _msgMap: {};
    ...
    public get protocolType(): cc_spark_socket_protocol { return this._socket.protocolType; }
     ...
     */
    public link(url: string, protocol: cc_spark_socket_protocol) {
        return new Promise<void>((resolve, reject) => {
            if (Assert.handle(Assert.Type.CreateObjectException, this._socket, "游戏Socket对象")) {
                this._url = url;
                switch(protocol) {
                    case ProtocolType.ARRAY_BUFFER:
    ...
    /**重连 */
    public reconnect() {
        this._socket.link(this._url).then(() => {
            this.registerEvent();
        });
    }
     ...
     * @returns 返回指定的消息代理对象
     */
    public get<T extends ISocketMessage<any, any>>(messageName: string, _type: new () => T): cc_spark_socket_message<T> {
        return this.retrieveMsg(messageName) as cc_spark_socket_message<T>;
    }
    ...
    public dispatchData(data: IResponseSocketData) {
        const proxy = this.retrieveMsg(data.proxyName) as ISocketMessage_extend;
        proxy.dispatch(data);
    }
