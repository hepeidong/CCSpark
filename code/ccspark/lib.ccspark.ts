...
export type cc_spark_static<T, U> = {
    new (): T;
} & U;
export type ggk_readonly_dictionary<T> = Readonly<Partial<T>>;
export type cc_spark_base_type<K, ConditionT, T1, T2> = K extends ConditionT ? T1 : T2;
export type cc_spark_key_base_type<K, T1, T2> = K extends T1 ? string : K extends T2 ? number : never;
export type cc_spark_map_base_type<T, K  extends number | string> = Record<K, T>;

export type cc_spark_vm_handler_opts<DataType> = { [P in keyof DataType]: (data:  DataType[P], key: P) => void }
export type cc_spark_vm_class<DataType> = cc_spark_static<any, {prototype: cc_spark_vm_handler_opts<DataType>}>;
export type cc_spark_loader_AssetType<T = Asset> = Constructor<T>;
export type cc_spark_initial_asset_info = {path: string, type: cc_spark_loader_AssetType}

export type cc_spark_win_model = "ROOT"|"DIALOG"|"ACTIVITY"|"TOAST"|"TOP";
export type cc_spark_win_activity = {priority: number, view: IWindowBase};

export type cc_spark_red_dot_list = {parentId: number, childId: number};
...
export type cc_spark_alGraph_edge_type = ANode;

export type cc_spark_alGraph_value_type = string;

export interface VNode {
    data: cc_spark_alGraph_value_type;
    firstArc: cc_spark_alGraph_edge_type;
}

export type cc_spark_alGraph_vertex_type = VNode;
...
export interface IWindowBase extends IMediator {
    ...
    readonly winModel: cc_spark_win_model;
    iniViewType(): void;
    getViewType(): number;
...
export interface IActivityManager extends ILayerManager {
    ...
    popActivity(): cc_spark_win_activity;
}
...
export type cc_spark_ecs_entity = Readonly<IBaseEntity>;
...
export type cc_spark_http_message<Base> = Base extends IHttpMessage<infer Response_T, infer Request_T> ? IHttpMessage<Response_T, Request_T> : never;
...
export interface IHttpManager {
     ...
     * @returns 返回指定的消息协议对象
     */
    get<T extends IHttpMessage<any, any>>(proxyName: string, type: new () => T): cc_spark_http_message<T>;
}
...
export interface ISocketMessage_extend extends ISocketMessage<any, any> {
    dispatch(data: IResponseSocketData): void;
}
export type cc_spark_socket_message<Base> = Base extends ISocketMessage<infer Response_T, infer Request_T> ? ISocketMessage<Response_T, Request_T> : never;
export type cc_spark_socket_protocol = "arraybuffer"|"json";
export interface ISocketProtocol {
    encodingData(data: ISocketData<any>): any;
...
export interface ISocket {
    protocolType: cc_spark_socket_protocol;
...
export interface ISocketManager {
    ...
    /**消息协议类型 */
    readonly protocolType: cc_spark_socket_protocol;
    /**连接成功回调 */
     ...
     */
    link(url: string, protocol: BinaryType|"json"): Promise<void>;
    /**发起重连 */
    reconnect(): void;
    /**
    ...
    /**
     * 获取消息协议对象
     * @param messageName 消息名，即类的名称
     * @param type 消息类型
     * @returns 返回指定的消息协议对象
     */
    get<T extends ISocketMessage<any, any>>(messageName: string, type: new () => T): cc_spark_socket_message<T>;
}
export interface ICCSocketManager extends ISocketManager {
    encodingData(data: ISocketData): void;
    sendData(data: any): void;
    dispatchData(data: IResponseSocketData): void;
}
...
export interface ILoader {
    load<T extends Asset>(paths: string, type: cc_spark_loader_AssetType<T>, onProgress: (finish: number, total: number, item: AssetManager.RequestItem) => void, onComplete: (error: Error, assets: T) => void): void;
    load<T extends Asset>(paths: string[], type: cc_spark_loader_AssetType<T>, onProgress: (finish: number, total: number, item: AssetManager.RequestItem) => void, onComplete: (error: Error, assets: Array<T>) => void): void;
    ...
    load<T extends Asset>(paths: string, type: cc_spark_loader_AssetType<T>, onComplete?: (error: Error, assets: T) => void): void;
    load<T extends Asset>(paths: string[], type: cc_spark_loader_AssetType<T>, onComplete?: (error: Error, assets: Array<T>) => void): void;
    ...
    loadDir<T extends Asset>(dir: string, type: cc_spark_loader_AssetType<T>, onProgress: (finish: number, total: number, item: AssetManager.RequestItem) => void, onComplete: (error: Error, assets: Array<T>) => void): void;
    ...
    loadDir<T extends Asset>(dir: string, type: cc_spark_loader_AssetType<T>, onComplete: (error: Error, assets: Array<T>) => void): void;
    loadDir<T extends Asset>(dir: string, type: cc_spark_loader_AssetType<T>): void;
...
/**************************************音频，动画等特效播放管理类型定义********************************/
export type cc_spark_audio_type = { audio: IAudioBGM | IAudio; clip: AudioClip; played: boolean; bgm: boolean; audioID: number; callbacks: cc_spark_audio_saudio_resolved_type[] };
export type cc_spark_audio_saudio_resolved_type = { type: string; call: any }
export type cc_spark_animat_resolved_type = { call: Function; type: string; };
export type cc_spark_animat_frameAnimat_type = { props: IFrameAnimat, callbacks: cc_spark_animat_resolved_type[] };
export type cc_spark_animat_spineAnimat_type = { props: ISpineAnimat, callbacks: cc_spark_animat_resolved_type[] };
export type cc_spark_animat_dragonBonesAnimat_type = { props: IDragonBonesAnimat, callbacks: cc_spark_animat_resolved_type[] }
...
/***************************************/
export type cc_spark_bitLblImage_type = { spacing: number; sf: SpriteFrame; }

export type cc_spark_redDot_type = { parent: number[]; }
...
/**时间类型 */
export type cc_spark_utils_date_compare_type = { year: number; month: number; date: number; hours: number; }